...
${responseFormat}`;
    ...
    if (!Array.isArray(analysis.tags)) {
      ...
      if (analysis.tags.length > 0 && typeof analysis.tags[0] === 'string') {
        ...
        analysis.tags = analysis.tags.filter((tag: { name: string; importance: number }) => 
          tag.name && 
    ...
    if (analysis.similarActivities) {
      if (!Array.isArray(analysis.similarActivities)) {
        ...
        analysis.similarActivities = analysis.similarActivities.filter((activity: { title: string; similarity: number; explanation: string }) => {
